...
### Object Tracking
There is an in-built object tracker in CellProfiler which allows for tracking of objects across frames. Depending on how much individual objects move between individual frames, individual parameters in the _TrackObjects_ module might need to be modified as elaborated below: 

Under _Choosing a tracking method_, multiple tracking methodologies can be used. I find the Distance method the simplest to use and is effective for our sample, but you can also try other methods available in the drop-down menu shown in the image:
![image](https://user-images.githubusercontent.com/46695970/123042294-2ede4b80-d429-11eb-8bbe-7f0c798e842d.png)

A disadvantage of using the Distance, Overlap, and Fellow Neighbors tracking method is that they rely on the immediate previous frame to do successive tracking i.e. if there is a frame where your object is missing or cannot be tracked, this will mess up the tracking tool. LAP is known to be able to track objects across multiple frames but the optimization process is lengthy. It requires you to optimize 10 or so parameters which I have not gotten any success with for our sample. 

If you were to you Distance like I have, you might need to optimize the _Maximum pixel distance to consider matches_ value. I optimized this value based on trial and error. When doing the optimization, you might want to create a substack of images to test if the object tracking works before running the entire pipeline. Saving the color-coded images in the _TrackObjects_ function will allow you to manually inspect the accuracy of the object tracking before doing the data analysis.

### Property Measurement
You can also decide which properties of the nucleus and cytoplasm you would like to measure, and what measurements you want to export in your final spreadsheet. These can be changed under the _MeasureObjectIntensity_, _MeasureObjectIntensityDistribution_, _MeasureObjectNeighbors_,_MeasureObjectSizeShape_ and _ExportToSpreadsheet_ function. Currently, the program is coded to measure the Image Number, Object Number, Intensity, Eccentricity of shape, Area, Perimeter, Distance to Nearest Neighbour, Nearest Neighbour Object Number, Location for the nucleus and cytoplasm separately in all images.  You can add even more properties for measurement and for more objects of your choosing by looking at the _ExportToSpreadsheet_ module and selecting _Press Button to Select Measurements_.
